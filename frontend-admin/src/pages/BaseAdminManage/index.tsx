...
const BaseAdminManage: React.FC = () => {
  ...
  const fetchData = useCallback(async (page = 1) => {
    setLoading(true)
    try {
      const res = await api.get('/api/admin/users/base-admins', { params: { page, size: 10 } })
      const d = res.data?.data ?? res.data
      setData(d?.records ?? d?.list ?? [])
      setTotal(d?.total ?? 0)
    } catch {
      message.error('获取基础管理员列表失败')
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const openEdit = (record: User) => {
    ...
    form.setFieldsValue({ email: record.email, firstName: record.firstName, lastName: record.lastName })
    setDrawerOpen(true)
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editRecord) {
        await api.put(`/api/admin/users/base-admins/${editRecord.id}`, values)
        message.success('更新成功')
      } else {
        await api.post('/api/admin/users/staff', { ...values, roleId: 7 })
  ...
  const handleDelete = async (id: number) => {
    try {
      await api.delete(`/api/admin/users/base-admins/${id}`)
  ...
  return (
    <div className="page-card">
      <div className="page-header">
        <h3 className="page-title">基础管理员账号</h3>
        <p className="page-description">管理平台基础内容的管理员账号</p>
      </div>
      ...
      <Drawer
        title={editRecord ? '编辑基础管理员' : '新增基础管理员'}
        width={720}
        open={drawerOpen}
...
export default BaseAdminManage
