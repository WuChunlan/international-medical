...
const HACasesPage: React.FC = () => {
  ...
  const fetchData = useCallback(async (page = 1) => {
    setLoading(true);
    try {
      const res = await api.get('/api/hospital-admin/cases', { params: { page, size: 10 } });
      const d = res.data?.data ?? res.data;
      setData(d?.records ?? d?.list ?? []);
      setTotal(d?.total ?? 0);
    } catch { message.error('获取案例列表失败'); }
  ...
  const handleDelete = async (id: number) => {
    try {
      await api.delete(`/api/hospital-admin/cases/${id}`);
      message.success('删除成功');
      fetchData(current);
    } catch { message.error('删除失败'); }
  };

  const openModal = (record?: MedicalCase) => {
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (editRecord) {
        await api.put(`/api/hospital-admin/cases/${editRecord.id}`, values);
        message.success('更新成功，等待审核');
      } else {
        await api.post('/api/hospital-admin/cases', values);
  ...
  return (
    <div>
      <Row justify="space-between" align="middle" className="page-header-row">
        <Col><Title level={4} className="page-title">过往案例</Title></Col>
        <Col><Button type="primary" icon={<PlusOutlined />} onClick={() => openModal()}>新增案例</Button></Col>
      </Row>
      <Card className="page-card">
        <Table rowKey="id" columns={columns} dataSource={data} loading={loading}
          pagination={{ current, pageSize: 10, total, showTotal: (t) => `共 ${t} 条`, onChange: (p) => setCurrent(p) }} />
      </Card>
      <Modal title={editRecord ? '编辑案例' : '新增案例'} open={modalOpen}
