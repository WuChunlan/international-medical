...
const HAProductsPage: React.FC = () => {
  ...
  const openModal = (record?: SpecialProduct) => {
  ...
  return (
    <div>
      <Row justify="space-between" align="middle" className="page-header-row">
        <Col><Title level={4} className="page-title">产品管理</Title></Col>
        <Col><Button type="primary" icon={<PlusOutlined />} onClick={() => openModal()}>新增产品</Button></Col>
      </Row>
      <Card className="page-card">
        <Table rowKey="id" columns={columns} dataSource={data} loading={loading}
          pagination={{ current, pageSize: 10, total, showTotal: (t) => `共 ${t} 条`, onChange: (p) => setCurrent(p) }} />
      </Card>
      <Modal title={editRecord ? '编辑产品' : '新增产品'} open={modalOpen}
