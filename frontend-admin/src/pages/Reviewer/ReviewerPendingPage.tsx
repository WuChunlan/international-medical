...
import {
  Card, Tabs, Table, Button, Space, Modal, Input, message, Typography, Badge, Tag,
} from 'antd'
...
const ReviewerPendingPage: React.FC = () => {
  ...
  const tabs = [
    { key: 'hospitals',    label: <Badge count={hospitals.length}    size="small">医院</Badge>, table: <Table rowKey="id" size="middle" dataSource={hospitals    as AuditRecord[]} loading={loading} columns={[...baseColumns('nameZh'),  actionCol('hospitals')]}    pagination={false} /> },
    { key: 'doctors',      label: <Badge count={doctors.length}      size="small">医生</Badge>, table: <Table rowKey="id" size="middle" dataSource={doctors      as AuditRecord[]} loading={loading} columns={[...baseColumns('nameZh'),  actionCol('doctors')]}      pagination={false} /> },
    { key: 'equipments',   label: <Badge count={equipments.length}   size="small">设备</Badge>, table: <Table rowKey="id" size="middle" dataSource={equipments   as AuditRecord[]} loading={loading} columns={[...baseColumns('nameZh'),  actionCol('equipments')]}   pagination={false} /> },
    { key: 'environments', label: <Badge count={environments.length} size="small">环境</Badge>, table: <Table rowKey="id" size="middle" dataSource={environments as AuditRecord[]} loading={loading} columns={[...baseColumns('nameZh'),  actionCol('environments')]} pagination={false} /> },
    { key: 'cases',        label: <Badge count={cases.length}        size="small">案例</Badge>, table: <Table rowKey="id" size="middle" dataSource={cases        as AuditRecord[]} loading={loading} columns={[...baseColumns('titleZh'), actionCol('cases')]}        pagination={false} /> },
    { key: 'products',     label: <Badge count={products.length}     size="small">产品</Badge>, table: <Table rowKey="id" size="middle" dataSource={products     as AuditRecord[]} loading={loading} columns={[...baseColumns('nameZh'),  actionCol('products')]}     pagination={false} /> },
  ]
  ...
  return (
    <div className="page-card">
      <div className="page-header">
        ...
        <p className="page-description">审核医院管理员提交的内容</p>
      </div>
      <Card>
