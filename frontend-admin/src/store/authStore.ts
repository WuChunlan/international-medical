...
type Role = 'admin' | 'hospital_admin' | 'reviewer' | 'user' | 'customer_rep' | 'translation_admin' | 'base_admin' | null;
...
interface AdminAuthState {
  ...
  setAuth: (token: string, username: string, role: Role, hospitalId?: number | null, mustChangePassword?: boolean, userId?: number | null) => void;
...
export const useAdminAuthStore = create<AdminAuthState>((set) => ({
  ...
  userId: localStorage.getItem('admin_user_id')
    ? Number(localStorage.getItem('admin_user_id'))
    : null,
  mustChangePassword: localStorage.getItem('admin_must_change_pw') === '1',
  setAuth: (token, username, role, hospitalId = null, mustChangePassword = false, userId = null) => {
    ...
    if (userId != null) localStorage.setItem('admin_user_id', String(userId));
    else localStorage.removeItem('admin_user_id');
    localStorage.setItem('admin_must_change_pw', mustChangePassword ? '1' : '0');
    set({ token, username, role, hospitalId, userId, mustChangePassword });
  },
  ...
  logout: () => {
    ...
    set({ token: null, username: null, role: null, hospitalId: null, userId: null, mustChangePassword: false });
  },
}));
